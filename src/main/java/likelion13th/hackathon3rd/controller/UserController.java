...
import likelion13th.hackathon3rd.dto.*;
...
public class UserController {
    ...
    public ApiResponse<List<MeetListResponse>> getCreatedList(
            @PathVariable("id") Integer userId,
            @RequestParam(value = "sortBy", defaultValue = "dateTime") String sortBy,
            @RequestParam(value = "order", defaultValue = "desc") String order
    ) {
        List<MeetListResponse> data = meetQueryService.getCreatedList(userId, sortBy, order);
    ...
    public ApiResponse<List<MeetListResponse>> getJoinedList(
            @PathVariable("id") Integer userId,
            @RequestParam(value = "sortBy", defaultValue = "dateTime") String sortBy,
            @RequestParam(value = "order", defaultValue = "desc") String order
    ) {
        List<MeetListResponse> data = meetQueryService.getJoinedList(userId, sortBy, order);
